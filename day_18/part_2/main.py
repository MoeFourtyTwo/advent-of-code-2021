...
def main(input_file: str = "input.txt") -> None:
    with open(input_file) as f:
        snail_fish_numbers = list(f.read().splitlines() | select(json.loads))

    max_magnitude = 0
    for left, right in itertools.permutations(snail_fish_numbers, 2):
        magnitude = (SnailFishPair.parse(left) + SnailFishPair.parse(right)).magnitude
